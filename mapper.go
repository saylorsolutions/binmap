...
type ReadFunc func(r io.Reader, endian ByteOrder) error

// WriteFunc is a function that writes data to a binary target.
type WriteFunc func(w io.Writer, endian ByteOrder) error
...
type Mapper interface {
	...
	Read(r io.Reader, endian ByteOrder) error
	// Write data to a binary target.
	Write(w io.Writer, endian ByteOrder) error
}
...
func (m *mapper) Read(r io.Reader, endian ByteOrder) error {
	if m.read != nil {
...
func (m *mapper) Write(w io.Writer, endian ByteOrder) error {
	if m.write != nil {
...
func OverrideEndian(m Mapper, endian ByteOrder) Mapper {
	return Any(
		func(r io.Reader, _ ByteOrder) error {
			return m.Read(r, endian)
		},
		func(w io.Writer, _ ByteOrder) error {
			return m.Write(w, endian)
		},
	)
}
...
func (h *EventHandler) Read(r io.Reader, endian ByteOrder) (err error) {
	if h.BeforeRead != nil {
...
func (h *EventHandler) Write(w io.Writer, endian ByteOrder) (err error) {
...
func OnPanic(mapper Mapper, panicHandler func(any) error) Mapper {
	return Any(
		func(r io.Reader, endian ByteOrder) (err error) {
			defer func() {
				if r := recover(); r != nil {
		...
		func(w io.Writer, endian ByteOrder) (err error) {
			defer func() {
				if r := recover(); r != nil {
					err = _panicHandler(r, panicHandler)
				}
